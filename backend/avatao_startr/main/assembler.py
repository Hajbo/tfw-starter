...
class Assembler(metaclass=SingletonMeta):
    ...
    def __init_language_config(self, language, framework):
        ...
        self._language_config = import_module(
            f"tfw.starters.{language_folder}.language_config"
        ).LanguageConfig(language_folder, framework_folder)
    ...
    def __copy_base_to_working_directory(self) -> None:
        ...
        shutil.copytree(self._tfw_starter_source_path, self._tfw_starter_workdir_path)
    ...
    def __template_dockerfile(self):
        with open(
            os.environ.get("TFW_STARTER_DOCKERFILE_DESTINATION"), "w+"
        ) as dockerfile:
            dockerfile.write(
                render_template(
    ...
    def __template_supervisor_config(self):
        with open(
            os.environ.get("TFW_STARTER_SUPERVISOR_CONFIG_DESTINATION"), "w+"
        ) as dockerfile:
            dockerfile.write(
                render_template(
                    path_to_template_file=os.environ.get(
                        "TFW_STARTER_SUPERVISOR_CONFIG_SOURCE"
                    ),
    ...
    def __generate_zip(self) -> ZipFile:
        starter_zip = shutil.make_archive(
            self._file_name, "zip", self._tfw_starter_workdir_path
        )
        self.__cleanup_directory(self._tfw_starter_workdir_path)
        return starter_zip
