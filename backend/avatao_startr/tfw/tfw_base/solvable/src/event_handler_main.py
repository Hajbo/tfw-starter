...
from tfw.components.frontend import (
    ConsoleLogsHandler,
    MessageQueueHandler,
...
def main():
    ...
    # Web IDE backend
    ide_eh = eh_factory.build(IdeHandler(patterns=["/home/user/workdir/*"]))
    deploy_eh = eh_factory.build(DeployHandler())
    # Web shell backend
    terminal_eh = eh_factory.build(
        TerminalHandler(
    ...
    process_eh = eh_factory.build(
        ProcessHandler(supervisor_uri=TFWENV.SUPERVISOR_HTTP_URI)
    )
    # Sends live logs of webservice process to frontend
    processlog_eh = eh_factory.build(
        ProcessLogHandler(
    ...
    message_fsm_steps_eh = eh_factory.build(
        messageFSMStepsHandler, event_handler_type=FSMAwareEventHandler
    )
    # Catches special commands
    commands_eh = eh_factory.build(
        TestCommandsHandler(bashrc=f"/home/{TAOENV.USER}/.bashrc")
    )
