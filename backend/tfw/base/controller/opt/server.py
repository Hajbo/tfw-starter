...
class ControllerPostHandler(RequestHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        self.write(json.dumps({"solved": self.controller.fsm_in_accepted_state}))


if __name__ == "__main__":
    controller_eh = EventHandlerFactory().build(
        lambda *_: None, event_handler_type=FSMAwareEventHandler
    )

    route = os.environ["SECRET"].rstrip("/")
    application = Application(
        [(f"/{route}/?", ControllerPostHandler, {"controller": controller_eh})]
    )
